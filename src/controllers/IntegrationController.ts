import { logger, syncLogger, db, validateParams, sendResponseCustom, moment, nowWib,
...
class IntegrationController {
  ...
  async handleSubmitData(req: any, res: any) {
    try {
      syncLogger.info(`[SYNC-KLHK] Triggered via IntegrationController.handleSubmitData`);
   ...
   */
    async handleSendData(req: any, res: any) {
      let tmpData: any = []
      try {
        syncLogger.info(`------------------- SYNC DATA STARTED ------------------`)
        ...
        for (let i = 0; i < dataUser.length; i++) {
          syncLogger.info(`------------------- SYNC DATA STARTING ${i}/${dataUser.length} ------------------`)
          const elx = dataUser[i];
          
          let data = await db.select(db.raw(`u.api_key, u.secret_key, d.category, w.*
            from mqtt_datas w 
            inner join devices d on d.id_mesin = w.uuid 
            inner join users u on u.id = d.dinas_id 
          ...
          */
          ...
          if (data.length == 0) {
            syncLogger.info(`------------------- SYNC DATA IS EMPTY ------------------`)
            continue
          }
          ...
          /* OLD AGGREGATION WITHOUT CATEGORY
          let ctx: any = Object.values(
            data.reduce((acc: any, item: any) => {
              if (!acc[item.id_stasiun]) {
                acc[item.id_stasiun] = {
        ...
        /* OLD FINAL AGGREGATION WITHOUT CATEGORY
        let finalData: any = Object.values(
          ctxData.reduce((acc: any, item: any) => {
            if (!acc[item.id_stasiun]) {
              acc[item.id_stasiun] = {
        ...
        */
        syncLogger.info(`------------------- SYNC DATA finalData : ${finalData.length} ------------------`)

        for (let z = 0; z < finalData.length; z++) {
          const elz: any = finalData[z];
          syncLogger.info(`------------------- SYNC DATA PROCESSING ${z}/${finalData.length} ------------------`)
          
          let targetUrl = elz.category === 'DLH' ? process.env.URL_DLH : process.env.URL_KLH;
          if (!targetUrl) targetUrl = process.env.URL_KLHK; // Fallback
            ...
            */
            ...
            data: {
              "data" : {
                ...
                "Jam" : nowWib('HH:mm:ss'),
                "Suhu" : parseFloat((parseFloat(elz.temperature) / elz.count).toFixed(2)),
                "TDS" : parseFloat((parseFloat(elz.tds) / elz.count).toFixed(2)),
                "DO" : parseFloat((parseFloat(elz.do_) / elz.count).toFixed(2)),
                "PH" : parseFloat((parseFloat(elz.ph) / elz.count).toFixed(2)),
                "Turbidity" : parseFloat((parseFloat(elz.turbidity) / elz.count).toFixed(2)),
                "Kedalaman" : parseFloat((parseFloat(elz.waterlevel) / elz.count).toFixed(2)),
                "Nitrat" : parseFloat((parseFloat(elz['no3']) / elz.count).toFixed(2)),
                "Amonia" : parseFloat((parseFloat(elz['nh3n']) / elz.count).toFixed(2)),
                "COD" : parseFloat((parseFloat(elz.cod) / elz.count).toFixed(2)),
                "BOD" : parseFloat((parseFloat(elz.bod) / elz.count).toFixed(2)),
                "TSS" : parseFloat((parseFloat(elz.tss) / elz.count).toFixed(2))
                },
                "apikey" : elz.apikey,
          ...
          syncLogger.info(`------------------- SYNC SUBMIT DATA TO API MENLHK ------------------`)
          let result = await axios.request(options)
          syncLogger.info(`------------------- SYNC RESPONSE FROM API MENLHK ${JSON.stringify(result.data)} ------------------`)
          let statusCode = result.data.status ? result.data.status.statusCode : 401
    
          if (statusCode == 200) {
            syncLogger.info(`------------------- [SYNC-SUCCESS] UPDATE DATA WATERMONITORING ------------------`)
            await db('res_klhk')
              .insert({
            ...
            syncLogger.info(`------------------- [SYNC-SUCCESS] UPDATE SUCCESFULLY ------------------`)
          } else {
            syncLogger.info(`------------------- [SYNC-FAILED] UPDATE DATA WATERMONITORING ------------------`)
            await db('res_klhk')
            .insert({
              payload: JSON.stringify(options.data),
              data_uid: '-',
            ...
            syncLogger.info(`------------------- [SYNC-FAILED] UPDATE SUCCESFULLY ------------------`)
          }
        }

        for (let r = 0; r < tmpData.length; r += 1000) {
          const batch = tmpData.slice(r, r + 1000);
          syncLogger.info(`------------------- [SYNC-TMPDATA] Process Update mqtt_datas ------------------`)
          ...
          syncLogger.info(`------------------- [SYNC-TMPDATA] Success Update mqtt_datas ------------------`)
        }
        ...
        for (let r = 0; r < tmpData.length; r += 1000) {
          const batch = tmpData.slice(r, r + 1000);
          syncLogger.info(`------------------- tmpData : ${JSON.stringify(batch)} ------------------`)
          await db.table('mqtt_datas')
          .whereIn(`id`, batch)
          .update({
            is_success: true,
